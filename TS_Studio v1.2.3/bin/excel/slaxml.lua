...
function SLAXML:parse(xml,options)
	...
	local function closeElement()
		...
		if first then
			...
			if self._call.startElement then self._call.startElement(table.unpack(currentElement)) end
			if self._call.attribute then
			for i=1,currentAttributeCt do self._call.attribute(table.unpack(currentAttributes[i])) end end

			if match1=="/" then
				pop(nsStack)
				if self._call.closeElement then self._call.closeElement(table.unpack(currentElement)) end
			end
			return true
		end
	end
